...
- **Speaker assignment is preprocessing:** `scripts/assign_dialogue_speakers.py` chooses persistent `speaker_id` values. `scripts/materialize_speaker_assignments.py` rewrites those IDs into render JSON and a CosyVoice `voice_map` keyed by `speaker_id`, so the same role name can map to different speakers across dialogues. The renderer does not assign speakers.
- **Per-dialogue failure containment:** batch processing continues and successful outputs are kept. Duplicate current `dialogue_id` values abort before any render. Opt-in `--resume` skips verified complete dialogues and rerenders failed or incomplete ones at dialogue granularity. There is no turn-level resume, automatic in-process retry, or interruption checkpoint.
