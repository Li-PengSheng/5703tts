...
EXPECTED_HIGGS_SUPPORT = {
    "rate": "pipeline_postprocess",
    "pause_before_ms": "pipeline_timing",
    "pause_after_ms": "pipeline_timing",
    "arousal": "provisional_model_control",
    "coarse_affect": "provisional_model_control",
...
def test_higgs_capabilities_match_declared_semantics() -> None:
    assert registry.control_support("higgs") == EXPECTED_HIGGS_SUPPORT
...
def test_pipeline_timing_controls_are_never_reported_as_ignored() -> None:
    ...
    assert registry.ignored_requested_controls("higgs", spec) == []
...
def test_higgs_reports_only_requested_unsupported_controls_as_ignored() -> None:
    spec = registry.requested_acoustic_spec(
        _Turn(
            rate="fast",
            pause_before_ms=800,
